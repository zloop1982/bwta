...
int main(int argc, char **argv)
{
  ...
  if (argc<2)
  {
    std::cout << "Please supply the file name of a map data file\n";
  ...
  load_map(filename,mapname,walkability);
  scene.setSceneRect(0,0, walkability.getWidth(), walkability.getHeight());
  ...
  {
    QPolygonF qp;
    for(int v=0;v<polygons[p].size();v++)
    {
      qp.append(QPointF(polygons[p].vertex(v).x(),polygons[p].vertex(v).y()));
    }
    scene.addPolygon(qp,QPen(QColor(0,255,0)));
    for(int v=0;v<polygons[p].size();v++)
    {
      scene.addEllipse(QRectF(polygons[p].vertex(v).x()-0.2,polygons[p].vertex(v).y()-0.2,0.4,0.4));
    }
  }
  cout << "extracted polygons from walkability data\n";
  ...
  cout << "inserting sites into voronoi diagram\n";
  SDG2 sdg;
  ...
  cout << "trimed off some blue edges\n";
  for (Arrangement_2::Vertex_iterator vit = arr.vertices_begin(); vit != arr.vertices_end(); ++vit)
  {
    if (vit->data().c==BLUE)
    {
  ...
  cout << "removed unnecessary blue verticies\n";
  for (Arrangement_2::Vertex_iterator vit = arr.vertices_begin(); vit != arr.vertices_end(); ++vit)
  {
    if (vit->data().c==BLUE)
    {
  ...
  cout << "identified chokepoint nodes\n";
...
//    scene.addLine(QLineF(x0,y0,(x0+x1)/2.0-8,(y0+y1)/2.0+4),QPen(color));
//    scene.addLine(QLineF((x0+x1)/2.0-8,(y0+y1)/2.0+4,x1,y1),QPen(color));
    scene.addLine(QLineF(x0,y0,x1,y1),QPen(color));
    //Segment seg(Point(x0,y0),Point(x1,y1));
  }
  /*
  for (Arrangement_2::Vertex_iterator vit = arr.vertices_begin(); vit != arr.vertices_end(); ++vit)
  {
    if (vit->data().c==BLACK)
    {
      ...
      scene.addEllipse(QRectF(x0-2,y0-2,4,4),QPen(QColor(0,0,0)));
    }
    else if (vit->data().c==BLUE)
    {
      int d=vit->degree()+1;
      double x0=cast_to_double(vit->point().x());
      double y0=cast_to_double(vit->point().y());
      scene.addEllipse(QRectF(x0-2*d,y0-2*d,4*d,4*d),QPen(QColor(0,0,255)));
    }
    else if (vit->data().c==RED)
    {
      double x0=cast_to_double(vit->point().x());
      double y0=cast_to_double(vit->point().y());
      scene.addEllipse(QRectF(x0-2,y0-2,4,4),QPen(QColor(255,0,0)));
    }
    else
    {
      double x0=cast_to_double(vit->point().x());
      double y0=cast_to_double(vit->point().y());
      scene.addEllipse(QRectF(x0-2,y0-2,4,4),QPen(QColor(0,255,0)));
      //stringstream ss;
      //ss << vit->degree();
      //scene.addText(QString(ss.str().c_str()));
    }
  }
  */
  for(std::set<Node*>::iterator r=g.regions_begin();r!=g.regions_end();r++)
  ...
  for(std::set<Node*>::iterator r=g.regions_begin();r!=g.regions_end();r++)
  {
    ...
    scene.addEllipse(QRectF(x0-2,y0-2,4,4),QPen(QColor(0,255,0)),QBrush(QColor(0,255,0)));
